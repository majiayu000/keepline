...
import { KEEPLINE_PID } from '../lib/paths.js';
...
export function isDaemonRunning(): boolean {
  if (!existsSync(KEEPLINE_PID)) {
    return false;
  }

  try {
    const pid = parseInt(readFileSync(KEEPLINE_PID, 'utf-8').trim(), 10);
    if (!validatePid(pid)) {
      cleanupPidFile();
      return false;
    }
...
function writePidFile(pid: number): void {
  writeFileSync(KEEPLINE_PID, String(pid));
}

/** Clean up PID file */
function cleanupPidFile(): void {
  if (existsSync(KEEPLINE_PID)) {
    unlinkSync(KEEPLINE_PID);
  }
}
...
export function startDaemon(): { success: boolean; pid?: number; error?: string } {
  ...
  try {
    ...
    const child = spawn(process.execPath, [process.argv[1], 'daemon', 'run'], {
      detached: true,
      stdio: 'ignore',
      env: { ...process.env, KEEPLINE_DAEMON: '1' },
    });
